...
//}
//
//func (n *BucketNode) Wire(ctx context.Context, input graph.Input) (graph.Input, error) {
//	bucket, ok := input.Resource.(*resource.FileStoreResource)
//	if !ok {
//		return graph.Input{}, fmt.Errorf("error getting blobstore resource: %s", n.Bucket.Path)
//	}
//
//	item, err := input.Observable.First().Get()
//	if err != nil {
//		return graph.Input{}, errors.Wrapf(err, "error getting first item from observable")
//	}
//
//	var (
//		bucketData []byte
...
//		if err != nil {
//			return graph.Input{}, errors.Wrapf(err, "error marshaling input params")
//		}
//	}
//
//	b, cleanup, err := bucket.WithPath(n.Path)
//	if err != nil {
//		return graph.Input{}, errors.Wrapf(err, fmt.Sprintf("error connecting to bucket: %s", n.Path))
//	}
//	defer cleanup()
//
...
//}
//
//func (n *FileNode) Wire(ctx context.Context, input graph.Input) (graph.Input, error) {
//	fs, ok := input.Resource.(*resource.FileStoreResource)
//	if !ok {
//		return graph.Input{}, fmt.Errorf("error getting filestore resource: %s", n.File.Path)
//	}
//	u, err := url.Parse(fs.Url)
//	if err != nil {
//		return graph.Input{}, errors.Wrapf(err, "error parsing filestore url")
//	}
