...
func (c *Converter) convertEnumType(enum *descriptor.EnumDescriptorProto, converterFlags ConverterFlags) (jsonschema.Type, error) {
	...
	if opts := enum.GetOptions(); opts != nil && proto.HasExtension(opts, E_EnumOptions) {
		if opt := proto.GetExtension(opts, E_EnumOptions); opt != nil {
			if enumOptions, ok := opt.(*EnumOptions); ok {
...
func (c *Converter) convertFile(file *descriptor.FileDescriptorProto, fileExtension string) ([]*plugin.CodeGeneratorResponse_File, error) {
	...
	if len(file.GetMessageType()) == 0 {
		...
		for _, msgDesc := range file.GetMessageType() {
			...
			if opts := msgDesc.GetOptions(); opts != nil && proto.HasExtension(opts, E_MessageOptions) {
				if opt := proto.GetExtension(opts, E_MessageOptions); opt != nil {
					if messageOptions, ok := opt.(*MessageOptions); ok {
...
func (c *Converter) convert(request *plugin.CodeGeneratorRequest) (*plugin.CodeGeneratorResponse, error) {
	...
	for _, fileDesc := range request.GetProtoFile() {
		...
		if opts := fileDesc.GetOptions(); opts != nil && proto.HasExtension(opts, E_FileOptions) {
			if opt := proto.GetExtension(opts, E_FileOptions); opt != nil {
				if fileOptions, ok := opt.(*FileOptions); ok {
